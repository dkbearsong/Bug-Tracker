...
const sql = {
  ...
  updateProjDetails : (projId, itemId, table, item, extra) => 'UPDATE proj_' + table + 's SET project_id = ' + projId + ', ' + table + ' = \'' + cleanRegX(item) + '\' ' +  extra + " WHERE id = " + itemId + '; ',
  deleteProjDetails : (id, table) => 'DELETE FROM proj_' + table + 's WHERE id = ' + id + ';',
